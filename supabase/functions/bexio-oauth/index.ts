      ...
      try {
        ...
        try {
          ...
          return new Response(`<!DOCTYPE html><html><head><meta charset="utf-8" /><title>Connecting...</title><meta name="viewport" content="width=device-width, initial-scale=1" /></head><body><script>(function(){try{var payload={type:'BEXIO_OAUTH_SUCCESS',credentials:${JSON.stringify(
            creds
          )},timestamp:Date.now()};try{localStorage.setItem('bexio_oauth_success',JSON.stringify(payload));localStorage.setItem('bexio_oauth_ready','true');}catch(_){ }try{window.name='BEXIO_OAUTH:'+encodeURIComponent(JSON.stringify(payload));}catch(_){ }if(window.opener){try{window.opener.postMessage(payload,'*');}catch(_){};(function(){var hard=function(){try{window.open('','_self');window.close()}catch(_){}};var soft=function(){try{window.close()}catch(_){hard()}};setTimeout(soft,50);var i=setInterval(soft,300);setTimeout(function(){try{clearInterval(i)}catch(_){ } location.replace('${mainAppUrl}');},4000);}());}else{location.replace('${mainAppUrl}');}}catch(e){document.body.innerHTML='<h1>Authentication completed</h1><p>You can close this window.</p>';}})();</script></body></html>`, {
            headers: { ...corsHeaders, 'Content-Type': 'text/html' },
          });
